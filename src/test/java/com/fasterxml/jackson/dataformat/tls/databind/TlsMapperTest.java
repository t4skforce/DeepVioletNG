package com.fasterxml.jackson.dataformat.tls.databind;
...
class TlsMapperTest {
  ...
  private final static byte[] INVALID_RECORD = new byte[] {
      // Record Header
      (byte) 0x00, // type is 0x00 (Unassigned record)
      (byte) 0x03, (byte) 0x01, // protocol version is 3.1 (also known as TLS 1.0)
      (byte) 0x00, (byte) 0x00, // 0x00 (0) bytes of handshake message follows
  };
  ...
  void test() throws Exception {
    TlsMapper mapper = TlsMapper.builder().build();
    ByteArrayInputStream in = new ByteArrayInputStream(CLIENT_HELLO_TLS_1_0);

    AbstractRecord record = mapper.readerFor(AbstractRecord.class).with(TlsParser.Feature.EMPTY_STRING_AS_NULL).readValue(in);
    assertEquals(Handshake.class, record.getClass());

    Handshake handshake = (Handshake) record;
    assertEquals(TlsVersion.TLS_V1, handshake.getProtocol());

    HandshakeMessage message = handshake.getMessage();
    assertEquals(ClientHello.class, message.getClass());

    ClientHello hello = (ClientHello) message;
    assertEquals(TlsVersion.TLS_V1_2, hello.getLegacyVersion());
    assertEquals("AAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8=", hello.getRandom());
...
//    System.out.println(ymlmapper.writeValueAsString(handshake));
  ...
  @Test
  void testInvalidRecordType() throws Exception {
    TlsMapper mapper = TlsMapper.builder().build();
    ByteArrayInputStream in = new ByteArrayInputStream(INVALID_RECORD);
    AbstractRecord record = mapper.readValue(in, AbstractRecord.class);
    assertEquals(UnassignedRecord.class, record.getClass());
  }
